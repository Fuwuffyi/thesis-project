...
void EventSystem::OnKeyDown(const uint32_t key, const KeyCallback cb) {
   m_keyDownListeners[key].emplace_back(std::move(cb));
}
void EventSystem::OnKeyUp(const uint32_t key, const KeyCallback cb) {
   m_keyUpListeners[key].emplace_back(std::move(cb));
}
void EventSystem::OnKeyHeld(const uint32_t key, const KeyCallback cb) {
   m_keyHeldListeners[key].emplace_back(std::move(cb));
}
void EventSystem::OnMouseDown(const uint32_t button, const MouseCallback cb) {
   m_mouseDownListeners[button].emplace_back(std::move(cb));
}
void EventSystem::OnMouseUp(const uint32_t button, const MouseCallback cb) {
   m_mouseUpListeners[button].emplace_back(std::move(cb));
}
void EventSystem::OnMouseHeld(const uint32_t button, const MouseCallback cb) {
   m_mouseHeldListeners[button].emplace_back(std::move(cb));
}
void EventSystem::OnCursorPos(const CursorPosCallback cb) {
   m_cursorPosListeners.emplace_back(std::move(cb));
}
void EventSystem::OnResize(const ResizeCallback cb) {
   m_resizeListeners.emplace_back(std::move(cb));
}
...
void EventSystem::HandleKeyEvent(const uint32_t key, const uint32_t scancode, const uint32_t action,
   ...
   if (action == GLFW_PRESS) {
      ...
      if (auto it = m_keyDownListeners.find(key); it != m_keyDownListeners.end()) {
         for (const auto& cb : it->second)
            cb(key, scancode, mods);
      }
   } else if (action == GLFW_RELEASE) {
      m_pressedKeys.erase(key);
      if (auto it = m_keyUpListeners.find(key); it != m_keyUpListeners.end()) {
         for (const auto& cb : it->second)
            cb(key, scancode, mods);
      }
   }
...
void EventSystem::HandleMouseEvent(const uint32_t button, const uint32_t action,
   ...
   if (action == GLFW_PRESS) {
      ...
      if (auto it = m_mouseDownListeners.find(button); it != m_mouseDownListeners.end()) {
         for (const auto& cb : it->second)
            cb(button, mods);
      }
   } else if (action == GLFW_RELEASE) {
      m_pressedMouseButtons.erase(button);
      if (auto it = m_mouseUpListeners.find(button); it != m_mouseUpListeners.end()) {
         for (const auto& cb : it->second)
            cb(button, mods);
      }
   }
...
void EventSystem::ProcessHeldEvents() {
   for (uint32_t key : m_pressedKeys) {
      if (auto it = m_keyHeldListeners.find(key); it != m_keyHeldListeners.end()) {
         for (const auto& cb : it->second)
            cb(key, 0, 0);
      }
   }
   for (uint32_t button : m_pressedMouseButtons) {
      if (auto it = m_mouseHeldListeners.find(button); it != m_mouseHeldListeners.end()) {
         for (const auto& cb : it->second)
            cb(button, 0);
      }
   }
