...
VulkanTexture::VulkanTexture(const VulkanDevice& device, uint32_t width, uint32_t height, Format format, bool isDepth, uint32_t samples)
   :
   m_device(&device),
   m_width(width),
   m_height(height),
...
void VulkanTexture::CreateImageView() {
   ...
   viewInfo.subresourceRange.aspectMask = m_isDepth ? VK_IMAGE_ASPECT_DEPTH_BIT : VK_IMAGE_ASPECT_COLOR_BIT;
...
VkFormat VulkanTexture::ConvertFormat(const Format format) {
   switch (format) {
      ...
      case Format::Depth24: return VK_FORMAT_D24_UNORM_S8_UINT;
...
ITexture::Format VulkanTexture::GetFormat() const {
   return m_format;
}
